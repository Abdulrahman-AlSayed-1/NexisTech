 ...
 * @typedef {'full' | 'md'} BadgeRounded
 *
 * @typedef {Object} BadgeProps
 * @property {React.ReactNode} [children] - Content inside the badge
 * @property {BadgeVariant} [variant] - Explicit visual color style
 * @property {BadgeStatus | string} [status] - Domain status string to map automatically to color variant
 * @property {BadgeSize} [size='md'] - Sizing scale
 * @property {boolean} [dot=false] - Whether to render a live indicator dot
 * @property {BadgeRounded} [rounded='full'] - Border radius style
...
const VARIANTS = {
  ...
  gold:
    'bg-accent-gold/15 text-accent-gold-hover dark:text-accent-gold border-accent-gold/30',
  success:
  ...
  'solid-gold':
    'bg-accent-gold text-primary-dark font-black border-accent-gold-hover shadow-xs',
  custom: '',
  none: '',
}
...
/**
 * Common Badge Primitive
 * Consistently renders status pills, category tags, and discount badges with light/dark theme contrast.
 *
...
export default function Badge({
  ...
  const normalizedStatus = status ? String(status).toLowerCase().replace(/[\s_-]+/g, '') : null
