 ...
 * @property {boolean} isOpen - Whether the modal is actively visible
 * @property {() => void} onClose - Callback invoked when the user dismisses the modal
 * @property {string} [title] - Header title text
 * @property {React.ReactNode} children - Content body
 * @property {React.ReactNode} [footer] - Optional footer action buttons
 * @property {string} [maxWidth='max-w-md'] - Max width Tailwind constraint class
 * @property {boolean} [closeOnBackdrop=true] - Whether clicking the backdrop closes the modal
 */

/**
 * Common Accessible Modal Primitive
 * Traps focus, handles Escape key, locks background scroll, and supports backdrop dismiss.
 *
...
export default function Modal({
  ...
  return (
    ...
    >
      ...
      {/* Modal Surface */}
      <div
        className={`relative w-full ${maxWidth} max-h-[calc(100dvh-1.5rem)] sm:max-h-[calc(100dvh-3rem)] flex flex-col rounded-2xl bg-bg-card dark:bg-dark-bg-card border border-border-medium dark:border-primary-medium/30 shadow-2xl z-10 overflow-hidden transform transition-all duration-200 animate-scale-in my-auto`}
      >
        {/* Modal Header */}
        {(title || onClose) && (
          <div className="shrink-0 flex items-center justify-between px-4 sm:px-6 py-3.5 sm:py-4 border-b border-border-medium/60 dark:border-primary-medium/30 bg-bg-card/95 dark:bg-dark-bg-card/95 backdrop-blur-xs z-20">
            {title && (
