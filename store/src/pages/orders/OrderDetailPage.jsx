...
const statusStyles = {
  pending:
    'border-yellow-600 bg-yellow-500 text-yellow-950',
...
const getId = (order) => order?._id || order?.id || order?.id
...
export default function OrderDetailPage() {
  ...
  return (
    <main className="min-h-screen bg-bg-main px-4 py-6 text-text-primary sm:px-6 sm:py-8 lg:px-10 lg:py-10 dark:bg-[var(--color-dark-bg-main)]">
      <div className="mx-auto w-full max-w-5xl">
        <div className="mb-6 flex items-start justify-between gap-4 sm:mb-8">
          <div>
            ...
            <h1 className="text-2xl font-bold tracking-tight sm:text-3xl dark:text-[var(--color-border-light)]">
              Order Details
            </h1>
          ...
          <span className={`shrink-0 rounded-full  px-3 py-1.5 text-xs font-semibold text-text-light ${statusClass}`}>
            {status}
          </span>
        </div>

        <section className="rounded-2xl border border-border-light bg-bg-card p-5 shadow-sm sm:p-7 dark:bg-[var(--color-dark-bg-card)] dark:border-[var(--color-border-light)]">
          <div className="mb-7 flex items-center gap-2 text-sm font-bold text-primary-dark dark:text-[var(--color-accent-gold-hover)]">
            <Package className="h-4 w-4 text-text-gold" />
            Order Progress
          </div>

          {status === 'Cancelled' ? (
            <div className="flex items-center gap-3 rounded-xl bg-primary-dark/10 px-4 py-3 text-sm font-semibold text-primary-dark dark:text-[var(--color-border-light)] dark:bg-[var(--color-text-secondary)]">
        ...
        <section className="mt-5 rounded-2xl border border-border-light bg-bg-card p-5 shadow-sm sm:p-7 dark:bg-[var(--color-dark-bg-card)]">
          <div className="mb-5 flex items-center gap-2 text-sm font-bold text-primary-dark dark:text-[var(--color-accent-gold-hover)]">
              ...
              items.map((item, index) => (
                ...
                >
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <p className="min-w-0 truncate text-sm text-text-secondary dark:text-[var(--color-border-light)]">
                      {getItemName(item)}{' '}
                      <span className="text-xs text-text-secondary dark:text-[var(--color-border-light)]">
                        · Qty: {getItemQuantity(item)}
                      </span>
                    </p>
                  </div>
                  <p className="shrink-0 text-sm font-bold text-primary-dark dark:text-[var(--color-border-light)]">
        ...
        <div className="mt-5 grid gap-5 md:grid-cols-2">
          <section className="rounded-2xl border border-border-light bg-bg-card p-5 shadow-sm sm:p-6 dark:bg-[var(--color-dark-bg-card)]">
            <div className="mb-4 flex items-center gap-2 text-sm font-bold text-primary-dark dark:text-[var(--color-accent-gold-hover)]">
              <MapPin className="h-4 w-4 text-text-gold " />
              Shipping Address
            </div>
            <div className="space-y-1 text-sm leading-5 text-text-secondary dark:text-[var(--color-border-light)]">
          ...
          <section className="rounded-2xl border border-border-light bg-bg-card p-5 shadow-sm sm:p-6 dark:bg-[var(--color-dark-bg-card)]">
            ...
            <p className="text-sm text-text-secondary dark:text-[var(--color-border-light)]">
              {order.paymentMethod || order.paymentType || 'Cash'}
            </p>
            <div className="my-4 h-px bg-border-light" />
            <div className="flex items-center justify-between gap-4">
              <span className="text-sm font-semibold text-text-secondary dark:text-[var(--color-border-light)]">Total</span>
              <span className="text-lg font-bold text-primary-medium dark:text-[var(--color-accent-gold-hover)]">
            ...
            <p className="mt-2 text-xs text-text-secondary dark:text-[var(--color-border-light)]">
